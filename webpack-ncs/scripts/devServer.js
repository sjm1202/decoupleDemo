...
function changeAssets(req, filename, submodules, submoduleAssets, output) {
  ...
  const _$ = cheerio.load(html); // 本地
  const $ = cheerio.load(output);  // 线上
...
module.exports = function (submodules = [], submoduleAssets = {}, submoduleUrls = {}) {
  ...
  return {
    onListening: function (devServer) {
      ...
      const port = serverPort = devServer.server.address().port;
      console.log(`Local: http://localhost:${port}`);
    },
    compress: true,
    allowedHosts: 'all',
    // allowedHosts: [
    //   '.console.test262-qingzhou.com'
    // ],
    devMiddleware: {
    ...
    proxy: {
      // /apis/aaa  --->  http://console.test262-qingzhou.com/aaa
      // '/apis': {
      //   target: 'http://console.test262-qingzhou.com',
      //   pathRewrite: { '^/apis': '' },
      //   changeOrigin: true,
      // },
      // '/kubecube/proxy': {
      ...
      //   router(req) { // 动态target
      //       if (req && req.hostname) {
      //           // console.log(req.hostname)  //dev.console.test262-qingzhou.com
      //           return `${req.protocol}://${req.hostname.replace(/^dev\./ig, '')}`;  // http://console.test262-qingzhou.com
      //       }
      //       return ''
      //   },
      //   changeOrigin: true,
      ...
      //   target: '/',
      //   router(req) {
      //     return `${req.protocol}://${req.hostname}:${serverPort}/public/local_proxy-${packageJSON.name}/${packageJSON.name}.html`;
      //   },
      //   pathRewrite: { [`^/${packageJSON.name}`] : '' },
      //   changeOrigin: true,
      // },
